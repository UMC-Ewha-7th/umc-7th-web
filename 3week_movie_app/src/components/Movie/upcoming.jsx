...
const fetchUpComing = async ({ pageParam = 1 }) => {
    ...
    const response = await axios.get(`https://api.themoviedb.org/3/movie/upcoming?language=en-US&page=${pageParam}`, {
        headers: {
...
const Upcoming = () => {
    ...
    const {
        ...
        queryFn:() => fetchUpComing({pageParam: page}), // 데이터 페칭 함수
        keepPreviousDate: true, // lastPage.page가 lastPage.total_pages보다 작으면 다음 페이지 번호를 반환하고, 그렇지 않으면 undefined를 반환하여 페칭을 중단
    });
