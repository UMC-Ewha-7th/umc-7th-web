...
const fetchNowPlaying = async ({ pageParam = 1 }) => {
    ...
    const response = await axios.get(`https://api.themoviedb.org/3/movie/now_playing?language=en-US&page=${pageParam}`, {
        headers: {
...
const NowPlaying = () => {
    const {
        ...
        queryFn: fetchNowPlaying, // 데이터 페칭 함수
    ...
    // Intersection Observer 인스턴스를 저장할 ref를 생성

    // loadMoreRef는 관찰할 DOM 요소를 참조하며, 로딩 중이거나 다음 페이지가 없으면 아무 작업도 하지 않음
