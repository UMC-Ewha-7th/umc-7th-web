...
const MovieDetail = () => {
    ...
    const { data: movieDetail, isLoading: isDetailLoading, isError:isDetailError } = useCustomFetch(`/movie/${movieId}?language=en-US`);
    const {data:movieCast, isLoading: isCastLoading, isError:isCastError} = useCustomFetch(`/movie/${movieId}/credits`);  

    if (isDetailError || isCastLoading) {
